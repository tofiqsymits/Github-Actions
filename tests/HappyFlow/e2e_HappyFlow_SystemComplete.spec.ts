...
const APPLICANT_NAME_HF: string = faker.name.firstName() + " " + faker.name.firstName();
...
const SCHEDULED_APPOINTMENT_text: string = "Appt. Scheduled";
const RESCHEDULED_APPOINTMENT_text: string = "Appt. Rescheduled";
const CANCELLED_APPOINTMENT_text: string = "Appt. Cancelled";
...
test.describe("--- HAPPYFLOW COMPLETE SYSTEM ---", () => {
  ...
  test('Create New Applicant for Testing "Complete HappyFlow System"', async ({ browserName, baseURL }) => {
    ...
    await test.step(`Getting "Email Address" for Applicant : "${APPLICANT_NAME_HF}"... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
    ...
    await test.step(`Filling Applicant's Information... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
      await POM_private._landinpage.create_applicant_from_landing_page(APPLICANT_NAME_HF, APPLICANT_EMAIL);
    })

    await test.step(`Waiting for Applicant : "${APPLICANT_NAME_HF}" to be Created Successfully... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
  ...
  test.describe("--- PHONE SCREEN - SUIT ---", () => {
    ...
    test('Get "Phone Screen" Scheduling Appointment URL from mail ==> PRIVATE WINDOW', async ({ browserName }) => {
      ...
      await test.step(`Clicking Mail Subject : "${PHONE_SCREEN_INVITE_Subject}"... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
        await POM_private._inboxesmailpage.click_on_MailSubject(PHONE_SCREEN_INVITE_Subject, APPLICANT_NAME_HF);
      })
    ...
    test(`Validate Applicant's stage is changed to "Walk In Interview" ==> Trigger Testing`, async ({ browserName }) => {
      ...
      await test.step(`Validating Applicant : "${APPLICANT_NAME_HF}" Stage is "Walk In Interview"... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
        for (let i = 0; i < (await POM._applicantpagetv.all_Names.count()); i++) {
          if (await POM._applicantpagetv.all_Names.nth(i).textContent() === APPLICANT_NAME_HF) {
  ...
  test.describe("--- WALK IN INTERVIEW - SUIT ---", () => {
    ...
    test('Get "Walk In Interview" Scheduling Appointment URL from Profile', async ({ browserName }) => {
      ...
      await test.step(`Picking Applicant : "${APPLICANT_NAME_HF}" from Tableview Page... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
        await POM._applicantpagetv.pick_Applicant(APPLICANT_NAME_HF);
      })
    ...
    test(`Schedule an Appointment for "Walk In Interview" Stage`, async ({ browserName }) => {
      ...
      await test.step(`Validating Assertion with Page Title... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
        await expect(page).toHaveTitle(AFTER_SUBMISSION_TITLE);
      })
    ...
    test(`Perform random "Pass", "Fail", "No Show" Action inside Applicant's Profile ==> Trigger Testing`, async ({ browserName }) => {
      ...
      await test.step(`Validating Applicant : "${APPLICANT_NAME_HF}" Stage is Changed to "Drop Destination"... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
        for (let i = 0; i < (await POM._applicantpagetv.all_Names.count()); i++) {
          if (await POM._applicantpagetv.all_Names.nth(i).textContent() === APPLICANT_NAME_HF) {
  ...
  test.describe("--- DROP DESTINATION - SUIT ---", () => {
    ...
    test(`Schedule and Reschedule an Appointment for "Drop Destination" Stage`, async ({ browserName }) => {
      ...
      await test.step(`Now Picking Applicant : "${APPLICANT_NAME_HF}" from Tableview Page... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
        await POM._applicantpagetv.pick_Applicant(APPLICANT_NAME_HF);
      })
    ...
    test('Perform random "Pass", "Fail", "No Show" or "Canceled" Action ==> Trigger Testing', async ({ browserName }) => {
      ...
      await test.step(`Validating Applicant's stage is changed to "Internal Questionnaire - E2E"... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
        for (let i = 0; i < (await POM._applicantpagetv.all_Names.count()); i++) {
          if (await POM._applicantpagetv.all_Names.nth(i).textContent() === APPLICANT_NAME_HF) {
  ...
  test.describe('--- INTERNAL QUESTIONNAIRE E2E - SUIT ---', () => {
      ...
      test('Fill "Internal Questionnaire (Profile)"', async ({ browserName, userAgent }) => {
        ...
        if (browserName !== "firefox") {    //  this condition is used to skip below 2 steps, because in Firefox browser, at these steps, page - browser are crashing
          await test.step(`Validating "Internal Questionnaire (Profile)" Modals's Heading... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
            await expect(page.getByText(INTERNAL_QUESTIONNAIRE_PROFILE_HEADING_text + APPLICANT_NAME_HF)).toBeVisible();
          })
      ...
      test(`Fill "Internal Questionnaire - Interview Action"`, async ({ browserName, userAgent }) => {
        ...
        if (browserName !== "firefox") {    //  this condition is used to skip below 2 steps, because in Firefox browser, at these steps, page - browser are crashing
          await test.step(`Validating "Internal Questionnaire - Interview Action" Modals's Heading... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
            await expect(page.getByText(INTERNAL_QUESTIONNAIRE_INTERVIEW_ACTION_HEADING_text + APPLICANT_NAME_HF)).toBeVisible();
          })
      ...
      test.describe('External Questionnaire (Qualifying) - SUIT', () => {
        ...
        test('Validate Assertions with "Heading" and "Description"', async ({ browserName }) => {
          ...
          await test.step(`Clicking Mail Subject : "${EXTERNAL_QUESTIONNAIRE_QUALIFYING_text}"... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
            await POM_private._inboxesmailpage.click_on_MailSubject(EXTERNAL_QUESTIONNAIRE_QUALIFYING_text, APPLICANT_NAME_HF);
          })
          ...
          if (browserName !== "firefox") {    //  this condition is used to skip below 2 steps, because in Firefox browser, at these steps, page - browser are crashing
            await test.step(`Validating Assertion with Heading... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
              await expect(POM_private._applicantpagetv._heading_externalquestionnaire.getByText(EXTERNAL_QUESTIONNAIRE_QUALIFYING_heading + " " + APPLICANT_NAME_HF)).toBeVisible();
            })
      ...
      test.describe('External Questionnaire (Unqualifying) - SUIT', () => {
        ...
        test('Validate Assertions with "Heading" and "Description"', async ({ browserName }) => {
          ...
          await test.step(`Clicking Mail Subject : "${EXTERNAL_QUESTIONNAIRE_UNQUALIFYING_text}"... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
            await POM_private._inboxesmailpage.click_on_MailSubject(EXTERNAL_QUESTIONNAIRE_UNQUALIFYING_text, APPLICANT_NAME_HF);
          })
          ...
          if (browserName !== "firefox") {    //  this condition is used to skip below 2 steps, because in Firefox browser, at these steps, page - browser are crashing
            await test.step(`Validating Assertion with Heading... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
              await expect(POM_private._applicantpagetv._heading_externalquestionnaire.getByText(EXTERNAL_QUESTIONNAIRE_UNQUALIFYING_heading + " " + APPLICANT_NAME_HF)).toBeVisible();
            })
      ...
      test.describe('External Questionnaire (Qualifying & Unqualifying) - SUIT', () => {
        ...
        test('Validate Assertions with "Heading" and "Description"', async ({ browserName }) => {
          ...
          await test.step(`Clicking Mail Subject : "${EXTERNAL_QUESTIONNAIRE_QUALIFYING_AND_UNQUALIFYING_text}"... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
            await POM_private._inboxesmailpage.click_on_MailSubject(EXTERNAL_QUESTIONNAIRE_QUALIFYING_AND_UNQUALIFYING_text, APPLICANT_NAME_HF);
          })
          ...
          if (browserName !== "firefox") {    //  this condition is used to skip below 2 steps, because in Firefox browser, at these steps, page - browser are crashing
            await test.step(`Validating Assertion with Heading... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
              await expect(POM_private._applicantpagetv._heading_externalquestionnaire.getByText(EXTERNAL_QUESTIONNAIRE_QUALIFYING_AND_UNQUALIFYING_heading + " " + APPLICANT_NAME_HF)).toBeVisible();
            })
      ...
      test.describe('External Questionnaire (Triggers) - SUIT', () => {
        ...
        test('Validate Assertions with "Heading" and "Description"', async ({ browserName }) => {
          ...
          await test.step(`Clicking Mail Subject : "${EXTERNAL_QUESTIONNAIRE_TRIGGER_text}"... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
            await POM_private._inboxesmailpage.click_on_MailSubject(EXTERNAL_QUESTIONNAIRE_TRIGGER_text, APPLICANT_NAME_HF);
          })
          ...
          if (browserName !== "firefox") {    //  this condition is used to skip below 2 steps, because in Firefox browser, at these steps, page - browser are crashing
            await test.step(`Validating Assertion with Heading... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
              await expect(POM_private._applicantpagetv._heading_externalquestionnaire.getByText(EXTERNAL_QUESTIONNAIRE_TRIGGER_heading + " " + APPLICANT_NAME_HF)).toBeVisible();
            })
  ...
  test.describe('--- REQUEST DOCUMENTS - SUIT ---', () => {
    ...
    test('Get "Upload Documents" URL via Mail', async ({ browserName }) => {
      ...
      await test.step(`Clicking Mail Subject : ${REQUEST_DOCUMENT_Subject}... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
        await POM_private._inboxesmailpage.click_on_MailSubject(REQUEST_DOCUMENT_Subject, APPLICANT_NAME_HF);
      })
    ...
    test(`Validate Applicant's Stage is Changed to "Hired" ==> Trigger Testing`, async ({ browserName }) => {
      ...
      await test.step(`Validating Applicant : "${APPLICANT_NAME_HF}'s" Stage is "Hired"... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
        for (let i = 0; i < await (POM._applicantpagetv.all_Names).count(); i++) {
          if (await POM._applicantpagetv.all_Names.nth(i).textContent() === APPLICANT_NAME_HF) {
            await expect(POM._applicantpagetv.all_Stages.nth(i).getByText(HIRED_Stage)).toBeVisible();
            return;
    ...
    test('Delete all Attachments', async ({ browserName }) => {
      ...
      await test.step(`Picking Applicant : "${APPLICANT_NAME_HF}" from Tableview Page... via \x1b[32m[${browserName}]\x1b[0m`, async () => {
        await POM._applicantpagetv.navigate_and_pick_Applicant(APPLICANT_NAME_HF);
      })
